      ...
      <div className="min-h-full bg-background pb-20 animate-in slide-in-from-right duration-300">
         ...
         <div className="p-5 space-y-8">
            ...
            <section className="space-y-3">
               ...
               <Card className={`p-0 overflow-hidden border ${kycStatus === 'verified' ? 'border-green-500/20' :
                  kycStatus === 'pending' ? 'border-yellow-500/20' : 'border-white/5'
                  }`}>
                  <div className="p-4 flex items-center gap-4">
                     <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${kycStatus === 'verified' ? 'bg-green-500/10 text-green-500' :
...
const Toggle: React.FC<{ checked: boolean; onChange: (checked: boolean) => void }> = ({ checked, onChange }) => (
   <button
      ...
      className={`w-11 h-6 rounded-full relative transition-colors duration-200 ease-in-out border border-transparent ${checked ? 'bg-primary' : 'bg-white/10'
         }`}
   >
      <div className={`w-5 h-5 rounded-full bg-white shadow-sm absolute top-0.5 left-0.5 transition-transform duration-200 ${checked ? 'translate-x-5' : 'translate-x-0'
         }`} />
   </button>
);